  ...
  return (
    <section ref={targetRef} className="relative h-[400vh] bg-black">
      <div className="sticky top-0 h-screen flex items-center overflow-hidden">
        ...
        <motion.div style={{ x }} className="flex gap-8 sm:gap-12 md:gap-16 lg:gap-24 px-6 sm:px-8 md:px-[10vw] lg:px-[15vw]">
          {dashboards.map((item, index) => (
            <div
              key={index}
              className="group relative flex-shrink-0 w-[85vw] sm:w-[80vw] md:w-[65vw] lg:w-[55vw] aspect-video"
            >
              {/* Card Container */}
              <div className="relative h-full w-full rounded-2xl md:rounded-[40px] overflow-hidden border border-white/5 bg-white/[0.02] backdrop-blur-sm group-hover:border-white/20 transition-all duration-700">
                {/* Background Glow */}
                ...
                {/* Overlay Content */}
                <div className="absolute inset-0 bg-linear-to-t from-black via-black/20 to-transparent opacity-90 p-6 sm:p-8 md:p-12 lg:p-16 flex flex-col justify-end">
                  <motion.div
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.8, delay: 0.2 }}
                  >
                    <span className="text-white/80 font-black tracking-[0.3em] uppercase text-[10px] mb-4 sm:mb-6 block">
                      Module 0{index + 1}
                    </span>
                    <h3 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white mb-4 sm:mb-6 uppercase tracking-tighter">
                      {item.title}
                    </h3>
                    <p className="text-white/80 text-base sm:text-lg md:text-xl max-w-xl font-medium leading-relaxed group-hover:text-white/80 transition-colors duration-500">
                      {item.description}
                    </p>
                  </motion.div>
                </div>
        ...
        <div className="absolute bottom-8 sm:bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
          <div className="w-px h-8 sm:h-12 bg-linear-to-b from-primary/50 to-transparent" />
          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">
            Scroll to explore
          </span>
        </div>
      </div>
