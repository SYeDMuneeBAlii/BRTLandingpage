...
export default function Footer() {
  ...
  const socialLinks = [
    { name: 'Instagram', href: process.env.NEXT_PUBLIC_INSTAGRAM_URL || '#', icon: Instagram, color: 'hover:text-pink-500' },
    { name: 'LinkedIn', href: process.env.NEXT_PUBLIC_LINKEDIN_URL || '#', icon: Linkedin, color: 'hover:text-blue-500' },
    { name: 'WhatsApp', href: `https://wa.me/${(process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '').replace(/\+/g, '')}`, icon: MessageCircle, color: 'hover:text-emerald-500' },
  ];
  ...
  return (
    <footer className="relative bg-black text-white py-24 sm:py-32 md:py-48 px-6 sm:px-8 lg:px-12 border-t border-white/5 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="lg:col-span-6 flex flex-col justify-between h-full">
            ...
            <div className="flex flex-wrap gap-6 mt-auto">
              {socialLinks.map((social) => (
                <a key={social.name} href={social.href} target="_blank" rel="noopener noreferrer" className={`group flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 ${social.color} transition-all duration-500 hover:scale-105 hover:bg-white/10`} title={social.name}>
                  <social.icon size={20} />
          ...
          <div className="lg:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-16">
            <div className="space-y-10">
              ...
              <ul className="space-y-6">
                {footerLinks.explore.map((link) => (
                  <li key={link.name}>
                    <a href={link.href} className="group flex items-center justify-between text-xl md:text-2xl font-black text-white/60 hover:text-white transition-all duration-300">
                      <span>{link.name}</span>
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-8">
                ...
                <a href={`https://wa.me/${(process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '').replace(/\+/g, '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-4 px-8 py-4 bg-primary text-black rounded-full font-black uppercase tracking-widest hover:scale-105 transition-all duration-500 shadow-xl shadow-primary/20">
        ...
        <div className="mt-32 md:mt-48 pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">
